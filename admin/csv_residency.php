...
class csv extends mysqli
{
	...
	{	
		...
		while ($row = fgetcsv($file)) {
			...
			$value = "'". implode("','", $row) ."'";
			$q = "INSERT INTO clearance (user_id, fullname, date, contact, email, adds) VALUES(". $value .")";
			if ($this->query($q)) {
				$this->state_csv = true;
			}
			else {
	...
	public function export() {
		$this->state_csv = false;
		$q = "SELECT t.user_id, t.fullname, t.date, t.contact, t.email, t.adds FROM clearance as t";
		$run = $this->query($q);
		if ($run->num_rows > 0) {
			$fn = "csv_". uniqid() .".csv";
