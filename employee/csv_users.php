...
class csv extends mysqli
{
	...
	{	
		...
		while ($row = fgetcsv($file)) {
			...
			$value = "'". implode("','", $row) ."'";
			$q = "INSERT INTO users (image, firstname, lastname, username, password, address, gender, bday, email, contact) VALUES(". $value .")";
			if ($this->query($q)) {
				$this->state_csv = true;
			}
			else {
	...
	public function export() {
		$this->state_csv = false;
		$q = "SELECT t.image, t.firstname, t.lastname, t.username, t.password, t.address, t.gender, t.bday, t.email, t.contact FROM users as t";
		$run = $this->query($q);
		if ($run->num_rows > 0) {
			$fn = "csv_". uniqid() .".csv";
