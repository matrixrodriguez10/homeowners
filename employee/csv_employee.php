...
class csv extends mysqli
{
	...
	{	
		...
		while ($row = fgetcsv($file)) {
			...
			$value = "'". implode("','", $row) ."'";
			$q = "INSERT INTO employee (image, firstname, lastname, username, password, contact, bday, email, position, address) VALUES(". $value .")";
			if ($this->query($q)) {
				$this->state_csv = true;
			}
			else {
	...
	public function export() {
		$this->state_csv = false;
		$q = "SELECT t.image, t.firstname, t.lastname, t.username, t.password, t.contact, t.bday, t.email, t.position, t.address FROM employee as t";
		$run = $this->query($q);
		if ($run->num_rows > 0) {
			$fn = "csv_". uniqid() .".csv";
