...
class csv extends mysqli
{
	...
	{	
		...
		while ($row = fgetcsv($file)) {
			...
			$value = "'". implode("','", $row) ."'";
			$q = "INSERT INTO scheduling (name, date, time, location, contact, email) VALUES(". $value .")";
			if ($this->query($q)) {
				$this->state_csv = true;
			}
			else {
	...
	public function export() {
		$this->state_csv = false;
		$q = "SELECT t.name, t.date, t.time, t.location, t.contact, t.email FROM scheduling as t";
		$run = $this->query($q);
		if ($run->num_rows > 0) {
			$fn = "csv_". uniqid() .".csv";
